-- Fix admin deletion access by updating RLS policies
-- The current issue is that useSimpleAuth doesn't create Supabase sessions
-- so auth.uid() returns null, blocking admin operations

-- Update the delete_student_and_cleanup function to be more permissive for system operations
...
-- Update the delete_driver_and_cleanup function to be more permissive for system operations
